import{az as n,z as r,aD as o}from"./index-DyVezt0R.js";const S=n("menus",{state:()=>({menus:[{id:"bbq-outdoor",name:"BBQ Menu - Outdoor",venue:"Outdoor",basePrice:4e3,minPax:50,template:`WELCOME DRINK (Select One):
...
`}]}),actions:{initializeStore(){const a=localStorage.getItem("slaf-menus");if(a)try{this.menus=JSON.parse(a)}catch(e){console.error("Failed to parse saved menus",e)}const t=localStorage.getItem("slaf-template-blocks");if(t)try{this.templateBlocks=JSON.parse(t)}catch(e){console.error("Failed to parse saved template blocks",e)}},saveToStorage(){localStorage.setItem("slaf-menus",JSON.stringify(this.menus)),localStorage.setItem("slaf-template-blocks",JSON.stringify(this.templateBlocks))},addMenu(a){this.menus.push({...a,id:Date.now().toString()}),this.saveToStorage();try{const t=r();o().logAction({user:t.user?.name||"System",role:t.role||"system",action:"Created Menu",module:"Menu Management",details:`Created new menu package: ${a.name}`})}catch(t){console.error("Audit log failed",t)}},updateMenu(a,t){const e=this.menus.findIndex(i=>i.id===a);if(e!==-1){this.menus[e]={...this.menus[e],...t},this.saveToStorage();try{const i=r();o().logAction({user:i.user?.name||"System",role:i.role||"system",action:"Updated Menu",module:"Menu Management",details:`Modified menu package: ${t.name}`})}catch(i){console.error("Audit log failed",i)}}},deleteMenu(a){const t=this.menus.find(e=>e.id===a)?.name||"Unknown";this.menus=this.menus.filter(e=>e.id!==a),this.saveToStorage();try{const e=r();o().logAction({user:e.user?.name||"System",role:e.role||"system",action:"Deleted Menu",module:"Menu Management",details:`Deleted menu package: ${t}`,status:"warning"})}catch(e){console.error("Audit log failed",e)}},addTemplateBlock(a){this.templateBlocks.push({...a,id:Date.now().toString()+"-tb"}),this.saveToStorage()},updateTemplateBlock(a,t){const e=this.templateBlocks.findIndex(i=>i.id===a);e!==-1&&(this.templateBlocks[e]={...this.templateBlocks[e],...t},this.saveToStorage())},deleteTemplateBlock(a){this.templateBlocks=this.templateBlocks.filter(t=>t.id!==a),this.saveToStorage()}}});export{S as u};
