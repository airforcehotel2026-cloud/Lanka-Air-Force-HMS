import{n as e}from"./pinia-CTaNOrng.js";import{t}from"./auditStore-sb-LosIL.js";import{s as n}from"./index-D5kQqZ7j.js";var r=e(`menus`,{state:()=>({menus:[{id:`bbq-outdoor`,name:`BBQ Menu - Outdoor`,venue:`Outdoor`,kitchenCost:2e3,fbStatus:`PUBLISHED`,gasCharge:200,officeCharge:500,profitMargin:1300,basePrice:4e3,minPax:50,template:`WELCOME DRINK (Select One):
...
`}]}),actions:{initializeStore(){let e=localStorage.getItem(`slaf-menus`);if(e)try{this.menus=JSON.parse(e)}catch(e){console.error(`Failed to parse saved menus`,e)}let t=localStorage.getItem(`slaf-template-blocks`);if(t)try{this.templateBlocks=JSON.parse(t)}catch(e){console.error(`Failed to parse saved template blocks`,e)}},saveToStorage(){localStorage.setItem(`slaf-menus`,JSON.stringify(this.menus)),localStorage.setItem(`slaf-template-blocks`,JSON.stringify(this.templateBlocks))},addMenu(e){let r={...e,id:Date.now().toString(),kitchenCost:e.kitchenCost||0,fbStatus:e.fbStatus||`DRAFT`,gasCharge:e.gasCharge||0,officeCharge:e.officeCharge||0,profitMargin:e.profitMargin||0,basePrice:e.basePrice||0};this.menus.push(r),this.saveToStorage();try{let e=n();t().logAction({user:e.user?.name||`System`,role:e.role||`system`,action:`Created Menu`,module:`Menu Management`,details:`Created new menu package: ${r.name}`})}catch(e){console.error(`Audit log failed`,e)}},updateMenu(e,r){let i=this.menus.findIndex(t=>t.id===e);if(i!==-1){this.menus[i]={...this.menus[i],...r},this.saveToStorage();try{let e=n();t().logAction({user:e.user?.name||`System`,role:e.role||`system`,action:`Updated Menu`,module:`Menu Management`,details:`Modified menu package: ${r.name}`})}catch(e){console.error(`Audit log failed`,e)}}},deleteMenu(e){let r=this.menus.find(t=>t.id===e)?.name||`Unknown`;this.menus=this.menus.filter(t=>t.id!==e),this.saveToStorage();try{let e=n();t().logAction({user:e.user?.name||`System`,role:e.role||`system`,action:`Deleted Menu`,module:`Menu Management`,details:`Deleted menu package: ${r}`,status:`warning`})}catch(e){console.error(`Audit log failed`,e)}},addTemplateBlock(e){this.templateBlocks.push({...e,id:Date.now().toString()+`-tb`}),this.saveToStorage()},updateTemplateBlock(e,t){let n=this.templateBlocks.findIndex(t=>t.id===e);n!==-1&&(this.templateBlocks[n]={...this.templateBlocks[n],...t},this.saveToStorage())},deleteTemplateBlock(e){this.templateBlocks=this.templateBlocks.filter(t=>t.id!==e),this.saveToStorage()}}});export{r as t};